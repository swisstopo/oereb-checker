...
import ch.swisstopo.oerebchecker.utils.XmlErrorHandler;
...
public class Validator {
    ...
    private static final Schema cachedXmlSchema = initialiseXmlSchema();


    private static Schema initialiseXmlSchema() {
        try {
            List<StreamSource> xsdStreamSources = getAvailableXsdStreamSources();
            SchemaFactory factory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
            return factory.newSchema(xsdStreamSources.toArray(new Source[0]));
        } catch (SAXException e) {
            logger.error("Failed to compile XSD schemas", e);
            return null;
        }
    }
    ...
    private static List<StreamSource> getAvailableXsdStreamSources() {
        ...
        streamSources.add(new StreamSource(
                ResourceHelper.getResourceStream("ch/swisstopo/oereb/geometry.xsd"),
                "http://www.interlis.ch/geometry/1.0")
        );
        streamSources.add(new StreamSource(
                ResourceHelper.getResourceStream("ch/swisstopo/oereb/xmldsig-core-schema.xsd"),
                "http://www.w3.org/2000/09/xmldsig#")
        );
        streamSources.add(new StreamSource(
                ResourceHelper.getResourceStream("ch/swisstopo/oereb/Versioning.xsd"),
                "http://schemas.geo.admin.ch/V_D/OeREB/1.0/Versioning")
        );
        streamSources.add(new StreamSource(
                ResourceHelper.getResourceStream("ch/swisstopo/oereb/ExtractData.xsd"),
                "http://schemas.geo.admin.ch/V_D/OeREB/2.0/ExtractData")
        );
        streamSources.add(new StreamSource(
                ResourceHelper.getResourceStream("ch/swisstopo/oereb/Extract.xsd"),
    ...
    public static ValidatorResult checkXml(InputStream inputStream) {
        logger.trace("Starting XML XSD validation.");

        ValidatorResult result = new ValidatorResult();
        XmlErrorHandler xsdErrorHandler = new XmlErrorHandler();

        if (cachedXmlSchema == null) {
            result.IsValid = false;
            result.addMessage("XML Schema could not be initialized.");
            return result;
        }

        try {
            javax.xml.validation.Validator xmlValidator = cachedXmlSchema.newValidator();
    ...
    public static ValidatorResult checkPdf(InputStream inputStream) {
        logger.trace("Starting PDF/A compliance validation.");
